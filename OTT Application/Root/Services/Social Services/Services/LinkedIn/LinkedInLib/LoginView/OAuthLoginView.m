...
@interface OAuthLoginView(){
    //    LinkedInLoginViewBlock authenticateBlock;
    CancelBlock cancelBlock;
}

@property (nonatomic, strong) LinkedInLoginViewBlock authenticateBlock;
...
- (void)openActiveSecsionWithCompleteHandler:(LinkedInLoginViewBlock)block didCancel:(CancelBlock)cancel{
    self.authenticateBlock = block;
    cancelBlock = cancel;
    
    [[appDelegate window].rootViewController presentViewController:self animated:YES completion:^{
...
{
    if (self.authenticateBlock != nil) {
        self.authenticateBlock(nil, [error description]);
        self.authenticateBlock = nil;
        cancelBlock = nil;
    }
...
- (void)accessTokenFromProvider
{
    ...
    [fetcher fetchDataWithRequest:request
                         delegate:self
                didFinishSelector:nil//@selector(accessTokenResult:didFinish:) NO SELECTOR
                  didFailSelector:nil];//@selector(accessTokenResult:didFail:)];
}
...
- (void)accessTokenResult:(OAServiceTicket *)ticket didFinish:(NSData *)data
{
    ...
    else
    {
        ...
        //Store access token
        [self.accessToken storeInUserDefaultsWithServiceProviderName:LinkedInAccessTokenKey prefix:nil];
    }
    // Notify parent and close this view
    //    [[NSNotificationCenter defaultCenter]
    //     postNotificationName:@"loginViewDidFinish"
    //     object:self];
    if (self.authenticateBlock != nil) {
        self.authenticateBlock(self, nil);
        self.authenticateBlock = nil;
        cancelBlock = nil;
    }
