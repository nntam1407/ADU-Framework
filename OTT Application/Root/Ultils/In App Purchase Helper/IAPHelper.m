...
- (id)init {
    return [self initWithProductIdentifiers:nil];
}
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response {
    ...
    DLog(@"Received products results...");
...
- (void)provideContent:(NSString *)productIdentifier {
    ...
    DLog(@"Toggling flag for: %@", productIdentifier);
...
- (void)failedTransaction:(SKPaymentTransaction *)transaction {
    if (transaction.error.code != SKErrorPaymentCancelled) {
        DLog(@"Transaction error: %@ %ld", transaction.error.localizedDescription,(long)transaction.error.code);
    }
...
- (void)paymentQueue:(SKPaymentQueue *)queue restoreCompletedTransactionsFailedWithError:(NSError *)error {
    ...
    DLog(@"Transaction error: %@ %ld", error.localizedDescription,(long)error.code);
